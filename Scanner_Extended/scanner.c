...
Token* readIdentKeyword(void) {
	...
	while(currentChar != EOF && (charCodes[currentChar] == CHAR_LETTER || charCodes[currentChar] == CHAR_DIGIT || charCodes[currentChar] == CHAR_UNDERSCORE) && count < MAX_IDENT_LEN){
		/* 
...
Token* readNumber(void) {	
	...
	if(lenght >= MAX_IDENT_LEN){
		error(ERR_NUMBERTOOLONG, lineNo, colNo);
		return tk;
	}
	
	tk->string[count] = '\0';
	if(tk->tokenType == TK_NUMBER){
		tk->value = atoi(tk->string);
	}else if (tk->tokenType == TK_REAL){
		tk->value = atof(tk->string);
	}
...
Token *readNumber2(void){
	Token *tk = makeToken(TK_REAL, lineNo, colNo);
	int count = 2;
	tk->string[0] = '.';
	tk->string[1] = (char)currentChar;
...
Token* getToken(void) {
	...
	switch (charCodes[currentChar]) {	
		...
		case CHAR_LSBRACKETS: /* Doc duoc dau '[' */
		...
		case CHAR_RSBRACKETS: /* Doc duoc dau ']' */
