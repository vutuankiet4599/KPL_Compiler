...
void compileArguments(void) {
	if(lookAhead->tokenType == SB_LPAR){
		eat(SB_LPAR);
  		...
  		compileArguments2();
  		eat(SB_RPAR);
	}else if(lookAhead->tokenType == SB_SEMICOLON || lookAhead->tokenType == KW_END || lookAhead->tokenType == KW_ELSE
	|| lookAhead->tokenType == KW_TO || lookAhead->tokenType == KW_DO){
		/* 
			Follow(Statement, For Statement)
			vi du 
			x();
			x() end.
			if x() else
		...
		*/
		...
		/*
			Follow(arguments2)
		*/
	}else if(lookAhead->tokenType == SB_EQ || lookAhead->tokenType == SB_NEQ || lookAhead->tokenType == SB_LE 
	|| lookAhead->tokenType == SB_LT || lookAhead->tokenType == SB_GE || lookAhead->tokenType == SB_GT){
		/*
			Follow(Condition2)
			x() = 0
			x() != 0
...
void compileTerm2(void) {
	...
	}else if(lookAhead->tokenType == SB_QUESTION){
	}else{
