...
void compileAssignSt(void) {
  ...
  if(lookAhead->tokenType == SB_ASSIGN){
  	...
  	genST();
  }else if(lookAhead->tokenType == SB_AS_ADD){
  	eat(SB_AS_ADD);
  	genCV();
  	genLI();
  	expType = compileExpression();
  	...
  	genAD();
  	genST();
  }else if(lookAhead->tokenType == SB_AS_MUL){
  	eat(SB_AS_MUL);
  	genCV();
  	genLI();
  	expType = compileExpression();
...
void compileWhileSt(void) {
  ...
  do{
  	...
  	if(lookAhead->tokenType == SB_SEMICOLON){
  		...
  		if(lookAhead->tokenType == KW_BREAK){
	  		eat(KW_BREAK);
	  		breakInstruction = genJ(DC_VALUE);
		}else if(lookAhead->tokenType == KW_CONTINUE){
			eat(KW_CONTINUE);
			genJ(beginWhile);
		}else{
			compileStatement();
  ...
  genJ(beginWhile);
  updateFJ(fjInstruction, getCurrentCodeAddress());
  updateJ(breakInstruction, getCurrentCodeAddress());
  eat(KW_END);
